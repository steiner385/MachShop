/**
 * Work Order Service
 * Core business logic for work order management
 * Extracted from monolith - provides complete feature parity
 */
...
export class WorkOrderService {
  ...
  validateWorkOrder(request: CreateWorkOrderRequest): WorkOrderValidationError[] {
    ...
    if (request.quantityOrdered > config.businessRules.maxWorkOrderQuantity) {
      errors.push({
        field: 'quantityOrdered',
        message: `Quantity ordered cannot exceed ${config.businessRules.maxWorkOrderQuantity} per work order`,
  ...
  async createWorkOrder(request: CreateWorkOrderRequest, createdBy: string): Promise<WorkOrder> {
    ...
    const workOrder = await prisma.workOrder.create({
      data: {
        ...
        priority: request.priority || WorkOrderPriority.NORMAL,
   ...
   */
  async getWorkOrders(filters?: WorkOrderFilters): Promise<{ workOrders: WorkOrder[]; total: number }> {
    const where: any = {};
   ...
   */
  async releaseWorkOrder(workOrder: WorkOrder, releasedBy: string): Promise<WorkOrder> {
    ...
    if (!transitionResult.success) {
      throw new Error(transitionResult.reason || 'Cannot release work order');
    }
  ...
  /**
   * Starts work order production
   */
  async startWorkOrder(workOrder: WorkOrder, startedBy: string): Promise<WorkOrder> {
    const transitionResult = stateMachine.transition(
      workOrder.status,
      'START',
      workOrder
    ...
    if (!transitionResult.success) {
      throw new Error(transitionResult.reason || 'Cannot start work order');
    }
   ...
   */
  async completeWorkOrder(workOrder: WorkOrder, completedBy: string): Promise<WorkOrder> {
    const transitionResult = stateMachine.transition(
      workOrder.status,
      'COMPLETE',
      workOrder
    ...
    if (!transitionResult.success) {
      throw new Error(transitionResult.reason || 'Cannot complete work order');
    }
   ...
   * Cancels work order
   */
  async cancelWorkOrder(workOrder: WorkOrder, cancelledBy: string): Promise<WorkOrder> {
    const transitionResult = stateMachine.transition(
      workOrder.status,
      'CANCEL',
      workOrder
    ...
    if (!transitionResult.success) {
      throw new Error(transitionResult.reason || 'Cannot cancel work order');
    }
  ...
  calculateEstimatedCompletion(workOrder: WorkOrder, dailyCapacity: number): Date | null {
    ...
    const capacity = dailyCapacity || config.businessRules.defaultDailyCapacity;
    ...
    const daysToComplete = Math.ceil(remainingQuantity / capacity);
   ...
   */
  canCancelWorkOrder(workOrder: WorkOrder): CancellationCheck {
    const transitionResult = stateMachine.canTransition(
      workOrder.status,
      'CANCEL',
      workOrder
    );
    ...
    return {
      canCancel: transitionResult.allowed,
  ...
  async getDashboardMetrics(siteId?: string): Promise<DashboardMetrics> {
    ...
    return {
      ...
      overdueWorkOrders,
      completedThisMonth,
      averageCompletionTime: 5.2, // days (TODO: calculate from actual data)
      onTimeDeliveryRate: 94.5, // percentage (TODO: calculate from actual data)
      productionEfficiency: 87.3 // percentage (TODO: calculate from actual data)
    };
  }
  ...
  private transformToWorkOrder(prismaWorkOrder: any): WorkOrder {
    return {
      ...
      scheduledStartDate: prismaWorkOrder.scheduledStartDate,
      actualStartDate: prismaWorkOrder.actualStartDate,
      scheduledEndDate: prismaWorkOrder.scheduledEndDate,
      actualEndDate: prismaWorkOrder.actualEndDate,
